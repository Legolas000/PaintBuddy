...
class ArtsItemsController extends Controller
{
    ...
    public function chItemStatus($itID)
    {
        $ValItem = DB::table('itemorders')
            ...
            ->count();
        if($ValItem == 0) {
            //Add file deletion after completion during presentation.Add $res and chk if changes done to DB
//            $val= DB::table('items')->where('itID','=',$itID)->select('imName')->first();
//            $file_path = public_path("img/tempEng/".$val->imName);
//            if(File::exists($file_path))
//                File::delete($file_path);
            $chkUsed = DB::table('itemorders')->where('itID' , '=', $itID)->count();
            if($chkUsed == 0)
                DB::table('items')->where('itID','=',$itID)->delete();
            else {
